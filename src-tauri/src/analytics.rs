...
pub fn get_analytics_data(days: Option<u32>) -> Result<AnalyticsData, String> {
    ...
    for record in &filtered {
        let entry = script_map
            .entry(record.script_name.clone())
            .or_insert((0, 0, 0, record.category.clone()));
    ...
    let mut top_scripts: Vec<ScriptStat> = script_map
        .into_iter()
        .map(|(name, (executions, total_duration, successes, category))| ScriptStat {
            name,
            executions,
...
pub fn clear_analytics_data() -> Result<(), String> {
    ...
    if file_path.exists() {
        fs::remove_file(&file_path)
            .map_err(|e| format!("Failed to delete analytics file: {}", e))
    } else {
        Ok(())
    }
}
