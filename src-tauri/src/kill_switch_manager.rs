...
pub async fn schedule_block(until: String, reason: String) -> Result<String, String> {
    ...
    chrono::DateTime::parse_from_rfc3339(&until)
        .map_err(|e| format!("Invalid timestamp format: {}. Use ISO 8601 (e.g., 2026-02-10T12:00:00Z)", e))?;
    ...
    log::info!("Kill switch scheduled until {} with reason: {}", until, reason);
...
pub async fn set_custom_message(message: String, redirect_url: Option<String>) -> Result<String, String> {
    let mut config = fetch_current_config().await?;
...
pub async fn push_config_to_github(mut config: KillSwitchConfig) -> Result<(), String> {
    let github_token = env::var("GITHUB_TOKEN")
        .map_err(|_| "GITHUB_TOKEN environment variable not set. Admin operations require a GitHub personal access token.".to_string())?;
    ...
    let current_file: serde_json::Value = current_file_response.json().await
        .map_err(|e| format!("Failed to parse current file response: {}", e))?;
    
    let sha = current_file["sha"].as_str()
...
pub fn create_local_override(allow: bool) -> Result<String, String> {
    let override_file = if let Some(data_dir) = dirs::data_dir() {
        data_dir.join("ScriptRunner").join("cache").join("kill_switch_override.json")
    } else {
    ...
    std::fs::write(&override_file, serde_json::to_string_pretty(&override_data).unwrap())
